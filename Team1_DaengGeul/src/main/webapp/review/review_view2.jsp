...
<style type="text/css">
	...
	.review_like_done_done {
	  	color: blue;	
	}
	.review_like_done_done:hover {
	  	color: gray;
	}
...
$(function() {
	...
	if('${review.review_like_done}'=='Y') {
		$(".review_like_done").prop("class","review_like_done_done");
	}
	
	$(".review_like_done").click(function() {
		$(this).toggleClass("review_like_done_done");
	});
	
	$(".review_like_done_done").click(function() {
		$(this).toggleClass("review_like_done");
	});
	...
	<section id="articleForm">
		...
		<section id="basicInfoArea">
			...
			<tr>
				...
				<!-- 3. jsp 파일에서 Y면 class=review_likedone 버튼 review_like_done=Y,
				N이면  class=review_like 버튼이되 링크는 ReviewLikeUpdate.bo? 보낼때 review_like_done=N로 보내기 -->				
					<c:choose>
						<c:when test="${review.review_like_done} == 'Y'">
							<button type="button" id="review_like_done" class="review_like_done" 
						...
						<c:otherwise>
							<button type="button" id="review_like_done" class="review_like_done_done" 
							onclick="location.href='ReviewLikeUpdate.re?review_idx=${param.review_idx}&member_id=${sessionScope.sId}&pageNum=${param.pageNum}&review_like_done=${review.review_like_done}'">
							<i class='far fa-thumbs-up' style='font-size:28px' ></i></button><div class="like_count"></div>
						</c:otherwise>
					</c:choose>
